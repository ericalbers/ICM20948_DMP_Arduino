...
int inv_icm20948_initialize(struct inv_icm20948 * s, const uint8_t *dmp3_image, uint32_t dmp3_image_size)
{
	...
	else 
	{
		/* Hardware initialization */
		// No image to be loaded from flash, no pointer to pass.
		if (rval=inv_icm20948_initialize_lower_driver(s, SERIAL_INTERFACE_I2C, dmp3_image, dmp3_image_size)) 
		{
			return rval; //-3;
		}
	}
...
{
	int rval = 0;
	rval = inv_icm20948_set_slave_compass_id(s, s->secondary_state.compass_slave_id);
	return rval;
		//return -4;
	//return 0;
}
